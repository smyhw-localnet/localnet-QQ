...
namespace Native.App.Export
{
	...
	{	
		...
		{	
			return "9,online.smyhw.localnet.KQ";	
		}	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("online.smyhw.localnet.KQ", 1, 9, "localnet 酷Q 连接插件", "1.0.0", 1, "smyhw", "localnet酷Q端连接插件", authCode);	
			appDataType.GetRuntimeProperty ("CQApi").GetSetMethod (true).Invoke (null, new object[] { new CQApi (appInfo) });	
			AppData.UnityContainer.RegisterInstance<CQApi> ("online.smyhw.localnet.KQ", AppData.CQApi);	
			// 向容器注册一个 CQLog 实例	
			appDataType.GetRuntimeProperty ("CQLog").GetSetMethod (true).Invoke (null, new object[] { new CQLog (authCode) });	
			AppData.UnityContainer.RegisterInstance<CQLog> ("online.smyhw.localnet.KQ", AppData.CQLog);	
